$(document).ready(function(){
	...
	$('.btnCapNhatThongTin').on("click", function(event) {
		...
		var href = "http://localhost:8080/phoneshop/api/profile/"+nguoiDungId;
	...
	function ajaxPostChangePass() {
	    	// PREPATEE DATA
	    	 ...
	    	 $.ajax({
	 			...
	 			url : "http://localhost:8080/phoneshop/api/profile/doiMatKhau",
	 			data : JSON.stringify(data),
	 			success : function(response) {
	 				if(response.status == "success"){
	 					$('#doiMKModal').modal('hide');
	 					alert("Đổi mật khẩu thành công. Bạn phải đăng nhập lại để xác nhận");
	 					location.href = "http://localhost:8080/phoneshop/logout";
	 				} else {
	 			    	$('input').next().remove();
