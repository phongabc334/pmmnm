$(document).ready(function() {
	...
	function ajaxGet(page){		
		...
		$.ajax({
			...
			url: "http://localhost:8080/phoneshop/api/don-hang/all" + '?page=' + page,
	...
	function ajaxPostPhanCongDonHang(email, donHangId) { 
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/phoneshop/api/don-hang/assign?shipper="+email+"&donHangId="+donHangId,
...
//		event.preventDefault();
	...
	$(document).on('click', '.btnChiTiet', function (event){
		...
		var donHangId = $(this).parent().prev().children().val();	
//		console.log(donHangId);
		var href = "http://localhost:8080/phoneshop/api/don-hang/"+donHangId;
	...
	function ajaxPostXacNhanHoanThanh() { 
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/phoneshop/api/don-hang/update?donHangId="+$("#idDonHangXacNhan").val()+"&ghiChu="+$("#ghiChuAdmin").val(),
 			enctype: 'multipart/form-data',
			success : function(response) {
	...
	function ajaxPostHuyDon(donHangId) { 
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8080/phoneshop/api/don-hang/cancel?donHangId="+donHangId,
			success : function(response) {
